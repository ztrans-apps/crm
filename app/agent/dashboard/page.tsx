...
export default async function AgentDashboard() {
  ...
  const [
    ...
    supabase.from('conversations').select('*', { count: 'exact', head: true }).eq('assigned_agent_id', user?.id || ''),
    supabase.from('tickets').select('*', { count: 'exact', head: true }).eq('assigned_agent_id', user?.id || ''),
    supabase.from('tickets').select('*', { count: 'exact', head: true }).eq('assigned_agent_id', user?.id || '').eq('status', 'resolved'),
    supabase.from('tickets').select('*', { count: 'exact', head: true }).eq('assigned_agent_id', user?.id || '').in('status', ['open', 'in_progress'])
  ])

  const stats = [
    {
      title: 'Assigned Chats',
      value: assignedChats || 0,
      icon: MessageSquare,
      color: 'text-blue-600',
      bgColor: 'bg-blue-100'
    },
