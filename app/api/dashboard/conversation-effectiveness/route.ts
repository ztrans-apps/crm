...
export const GET = withAuth(async (req, ctx) => {
  ...
  let conversationsQuery = ctx.supabase
    .from('conversations')
    .select(`
      id,
      assigned_to,
  ...
  const { data: timeSeriesMetrics } = await ctx.supabase
    .from('conversation_metrics')
  ...
  // 4. Resolved vs Open
  let resolvedQuery = ctx.supabase
    .from('conversations')
  ...
  const { count: resolvedCount } = await ctx.supabase
    .from('conversations')
