...
export async function GET() {
  try {
    ...
    const { data: conversations24h } = await supabase
      .from('conversations')
      .select('created_at')
    ...
    const { data: contacts24h } = await supabase
      .from('conversations')
    ...
    const { data: conversations7days } = await supabase
      .from('conversations')
      .select('created_at')
    ...
    return NextResponse.json(
      { error: 'Failed to fetch customer load metrics' },
      { status: 500 }
    )
  }
}
