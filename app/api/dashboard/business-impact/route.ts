...
export const GET = withAuth(async (req, ctx) => {
  ...
  const { count: leadsThisMonth } = await ctx.supabase
    .from('contacts')
    .select('*', { count: 'exact', head: true })
    .or('tags.cs.{"lead"}')
    .gte('created_at', monthAgo.toISOString())

  const { count: leadsLastMonth } = await ctx.supabase
    .from('contacts')
    .select('*', { count: 'exact', head: true })
    .or('tags.cs.{"lead"}')
  ...
  // 2. Tickets resolved
  const { count: ticketsToday } = await ctx.supabase
    .from('tickets')
    .select('*', { count: 'exact', head: true })
    .eq('status', 'closed')
    .gte('updated_at', today.toISOString())

  const { count: ticketsWeek } = await ctx.supabase
    .from('tickets')
    .select('*', { count: 'exact', head: true })
    .eq('status', 'closed')
    .gte('updated_at', weekAgo.toISOString())

  const { count: ticketsMonth } = await ctx.supabase
    .from('tickets')
    .select('*', { count: 'exact', head: true })
    .eq('status', 'closed')
  ...
  const campaignConversion = await Promise.all(
    (campaigns || []).map(async (campaign) => {
      ...
      const { count: responded } = await ctx.supabase
        .from('messages')
        .select('*', { count: 'exact', head: true })
  ...
  const { data: allContacts } = await ctx.supabase
    .from('contacts')
    .select('id')
    .gte('created_at', monthAgo.toISOString())
  ...
  const { data: conversationCounts } = await ctx.supabase
    .from('conversations')
    .select('contact_id')
  ...
  const { data: lastMonthContacts } = await ctx.supabase
    .from('contacts')
    .select('id')
    .gte('created_at', twoMonthsAgo.toISOString())
    .lt('created_at', monthAgo.toISOString())
  ...
  const { data: lastMonthConversations } = await ctx.supabase
    .from('conversations')
    .select('contact_id')
  ...
  const { data: botMetrics } = await ctx.supabase
