...
function UnifiedChatsContent() {
  ...
  // Load quick replies
  const loadQuickReplies = async (uid: string) => {
    try {
      const data = await chatService.getOrCreateDefaultQuickReplies(uid)
  ...
  // Initialize quick replies when userId is available
  useState(() => {
    if (userId) {
  ...
  const handleSaveNote = async (content: string, rating: number | null) => {
    ...
    try {
      await chatService.saveNote(selectedConversation.id, content, rating, userId)
  ...
  return (
    <div className="h-full flex">
      ...
      <ConversationList
        ...
        loading={conversationsLoading}
        onPickConversation={conversationActions.canPick ? handlePickConversation : undefined}
